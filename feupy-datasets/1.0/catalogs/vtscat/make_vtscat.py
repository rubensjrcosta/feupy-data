...
def read_vtscat_sources(which="all", sources_path=SOURCES_PATH):
    ...
    if not filenames:
        raise FileNotFoundError(
            f"No matching files found for key '{which}' in directory '{sources_path}'."
        )
...
def make_catalog():
    ...
    table["veritas_components"].description = "VERITAS source components"
    table["other_names"].description = "Other names used in literature or catalogs"
    ...
    table["source_name"].description = (
        'Source name: "veritas_name" if available, else "common_name"'
    )
