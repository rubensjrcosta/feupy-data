...
"""Generate eHWC flux-points tables.

The flux points are taken from:
...
def make_flux_points_table(source_name, source_data):
    """Create an eHWC flux-points table."""
    ...
    e2dnde_ul = np.asarray(
        source_data.get(
            "e2dnde_ul",
            np.full_like(e2dnde, np.nan),
        )
    )
    ...
    if len(lengths) != 1:
        raise ValueError(f"Inconsistent flux-point data lengths for {source_name}")
...
def validate_flux_points_table(table):
    """Validate a flux-points table with Gammapy."""
    FluxPoints.from_table(table, sed_type=SED_TYPE)


def write_flux_points_tables(outdir=FLUX_POINTS_DIR):
    """Generate and write all eHWC flux-points tables."""
    outdir = Path(outdir)
