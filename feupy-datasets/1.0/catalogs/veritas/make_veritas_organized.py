...
def make_flux_points_table(source_name, source_data):
    ...
    table.meta["reference"] = source_data.get("reference", REFERENCE)
...
def make_dataset(source_name, source_data):
    """Create one FluxPointsDataset with its fitted model."""
...
def make_catalog_table(data):
    ...
    table["spectral_model"] = Column(
        data=[data[name].get("spectral_model", "") for name in names],
...
def write_outputs(data, output_dir="."):
    """Write catalog, datasets, and models files."""
    ...
    catalog.write(output_dir / "veritas.fits", format="fits", overwrite=True)
